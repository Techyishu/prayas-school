...
import { ArrowRight, CheckCircle2, GraduationCap, Briefcase, Users, Plane, Globe, Phone, FileText, Layout, Database, Code } from "lucide-react"
...
export default function ServicesClient({ services }: { services: any[] | null }) {
    const handleWhatsApp = () => {
        window.open("https://wa.me/919653505005?text=Hi, I'd like to inquire about your study abroad services", "_blank")
    }

    // Merge or fallback
    const displayServices = services || originalServices;
            ...
            <section className="py-24">
                <div className="container-custom space-y-24">
                    {displayServices.map((service, index) => {
                        // resolve icon name or use fallback
                        // @ts-ignore
                        const Icon = Icons[service.icon || service.iconName] || Icons.HelpCircle;
                        const isEven = index % 2 === 0

                        // Handle DB structure vs Static structure differences
                        // DB uses 'details' jsonb, Static has direct props
                        const countries = service.details?.countries || service.countries || [];
                        const features = service.details?.features || service.features || [];
                        const accordion = service.details?.accordion || service.accordion || [];
                        const gradient = service.gradient || "from-blue-500 to-indigo-600";
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`absolute ${isEven ? 'right-0' : 'left-0'} top-0 w-96 h-96 bg-gradient-to-br ${gradient} opacity-5 rounded-full blur-3xl -z-10`} />
